...
var svg = d3.select('#chart-1').append('svg').attr('height', height + margin.top + margin.bottom).attr('width', width + margin.left + margin.right).append('g').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')').append('g').attr('transform', "translate(".concat(width / 2, ",").concat(height / 2, ")"));
var pie = d3.pie().value(function (d) {
  return d.minutes;
});
var task = ['Reading StackOverflow', 'Typing code', 'Rewriting code'];
var radius = 135;
var arc = d3.arc().innerRadius(0).outerRadius(radius);
var labelArc = d3.arc().innerRadius(radius).outerRadius(radius + 20); //   .startAngle(d => angleScale(d))
...
function ready(datapoints) {
  ...
  .attr('alignment-baseline', 'middle').attr('transform', function (d) {
