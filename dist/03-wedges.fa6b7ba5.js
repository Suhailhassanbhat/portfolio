...
var svg = d3.select('#chart-3').append('svg').attr('height', height + margin.top + margin.bottom).attr('width', width + margin.left + margin.right).append('g').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')').append('g').attr('transform', "translate(".concat(width / 2, ",").concat(height / 2, ")")); // const pie = d3.pie().value(1 / 12)
...
function ready(datapoints) {
  ...
  radiusScale.domain(d3.extent(temps)); // angleScale.domain(d3.extent(months))
  ...
  svg.append('text').text('NYC high temperatures, by month').style('font-weight', 'bold').style('font-size', 20).attr('x', 0).attr('text-anchor', 'middle').attr('y', -height / 2 + 40);
}
