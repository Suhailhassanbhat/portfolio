...
var svg = d3.select('#chart-2').append('svg').attr('height', height + margin.top + margin.bottom).attr('width', width + margin.left + margin.right).append('g').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')'); //   .append('g')
...
function ready(datapoints) {
  ...
  svg.selectAll('.pie').data(nested).enter().append('g').each(function (d) {
    ...
    container.selectAll('.path').data(pie(datapoints)).enter().append('path').attr('d', function (d) {
      ...
      var xPosition = xPositionScale(d.data.project);
      return "translate(".concat(xPosition, ", ").concat(height / 2, ")");
    });
    container.selectAll('.outside-label').data(pie(datapoints)).enter().append('text').text(function (d) {
      return d.data.project;
    }).attr('text-anchor', 'middle').attr('alignment-baseline', 'middle').attr('transform', function (d) {
      var xPosition = xPositionScale(d.data.project);
      return "translate(".concat(xPosition, ", ").concat(height - 50, ")");
    });
  });
}
