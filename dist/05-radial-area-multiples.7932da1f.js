...
var months = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sept', 'Oct', 'Nov', 'Dec', 'Jan'];
var angleScale = d3.scaleBand().domain(months).range([0, Math.PI * 2]);
var radius = 80;
var radiusScale = d3.scaleLinear().domain([0, 100]).range([22, radius]);
...
var xPositionScale = d3.scalePoint().range([radius, width - radius]);
...
function ready(datapoints) {
  ...
  xPositionScale.domain(cities); // const maxTemp = datapoints.map(function(d) {
  //   return +d.high_temp
  // })
  ...
  console.log(nested);
  svg.selectAll('.radial-area').data(nested).enter().append('g').each(function (d) {
    var container = d3.select(this);
    var datapoints = d.values;
    var city = d.key;
    datapoints.push(datapoints[0]);
    console.log(datapoints);
    container.append('path').datum(datapoints).attr('d', function (d) {
      return line(d);
    }).attr('fill', 'pink').attr('stroke', 'none').attr('transform', function (d) {
      var xPosition = xPositionScale(city);
      return "translate(".concat(xPosition, ", ").concat(height / 2, ")");
    });
    var bands = [20, 40, 60, 80, 100];
    container.selectAll('.band').data(bands).enter().append('circle').attr('fill', 'none').attr('stroke', 'grey').attr('r', function (d) {
      return radiusScale(d);
    }).attr('transform', function (d) {
      var xPosition = xPositionScale(city);
      return "translate(".concat(xPosition, ", ").concat(height / 2, ")");
    });
    container.append('text').text(city).attr('text-anchor', 'middle').style('font-weight', 'bold').style('font-size', 12).attr('transform', function (d) {
      var xPosition = xPositionScale(city);
      return "translate(".concat(xPosition, ", ").concat(height / 2, ")");
    });
    var labels = [20, 60, 100];
    container.selectAll('.label').data(labels).enter().append('text').text(function (d) {
      return "".concat(d, "\xB0");
    }).attr('y', function (d) {
      return -radiusScale(d) - 2;
    }).attr('text-anchor', 'middle').attr('alignment-baseline', 'middle').style('font-size', 7).attr('transform', function (d) {
      var xPosition = xPositionScale(city);
      return "translate(".concat(xPosition, ", ").concat(height / 2, ")");
    });
  });
  svg.append('text').text('Average Monthly Temperatures').style('font-weight', 'bold').style('font-size', 20).attr('x', width / 2).attr('text-anchor', 'middle').attr('y', margin.top / 2);
  svg.append('text').text('in cities around the world') // .style('font-weight', 'bold')
  .style('font-size', 12).attr('x', width / 2).attr('text-anchor', 'middle').attr('y', margin.top / 2 + 15);
}
