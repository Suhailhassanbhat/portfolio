...
function ready(_ref) {
  ...
  svg.append('path').datum({
    type: 'Sphere'
  }).attr('d', path).attr('fill', '#87ceeb');
  svg.selectAll('path').data(countries.features).enter().append('path').attr('class', 'country').attr('d', path).attr('fill', 'lightgrey').attr('stroke', 'black');
  svg.selectAll('.city').data(coordinateData).enter().append('circle').attr('class', 'city').attr('r', 2).attr('fill', 'white').attr('transform', function (d) {
    return "translate(".concat(projection([d.longitude, d.latitude]), ")");
  });
