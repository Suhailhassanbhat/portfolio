...
var radius = 150;
var radiusScale = d3.scaleLinear().domain([0, 90]).range([20, radius]);
...
function ready(datapoints) {
  ...
  svg.append('path').datum(datapoints).attr('d', line).attr('fill', 'lightblue').attr('stroke', 'none').attr('opacity', 0.8);
  svg.append('text').text('NYC').attr('x', 0).attr('y', 9).attr('text-anchor', 'middle').style('font-weight', 'bold').style('font-size', 24);
  ...
  var labels = [30, 50, 70, 90];
  svg.selectAll('.label').data(labels).enter().append('text').text(function (d) {
    return "".concat(d, "\xB0");
  }).attr('y', function (d) {
    return -radiusScale(d) - 3;
  }).attr('text-anchor', 'middle').attr('alignment-baseline', 'middle').style('font-size', 8);
}
