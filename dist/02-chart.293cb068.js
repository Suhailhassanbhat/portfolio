...
function ready(datapoints) {
  ...
  svg.selectAll('path').data(nested).enter().append('path').attr('class', 'mypath').attr('d', function (d) {
    return line(d.values);
  }).attr('stroke', function (d) {
    return colorScale(d.key);
  }).attr('stroke-width', 2).attr('fill', 'none');
  svg.selectAll('circle').data(nested).enter().append('circle').attr('class', 'mycircle').attr('fill', function (d) {
    return colorScale(d.key);
  ...
  svg.selectAll('text').data(nested).enter().append('text').attr('class', 'mytext').attr('y', function (d) {
    return yPositionScale(d.values[0].price);
  }).attr('x', function (d) {
    return xPositionScale(d.values[0].datetime);
  }).text(function (d) {
  ...
  svg.append('text').attr('font-size', '24').attr('text-anchor', 'middle').attr('class', 'main-text').text('U.S. housing prices fall in winter').attr('x', width / 2).attr('y', -40).attr('dx', 40);
  var rectWidth = xPositionScale(parseTime('February-17')) - xPositionScale(parseTime('November-16'));
  svg.append('rect').attr('x', xPositionScale(parseTime('December-16'))).attr('y', 0).attr('class', 'myrect').attr('width', rectWidth).attr('height', height).attr('fill', '#C2DFFF').lower();
  ...
  d3.select('#us-step').on('stepin', function () {
    ...
    svg.selectAll('.mycircle').attr('fill', function (d) {
      if (d.key === 'U.S.') {
        return 'red';
      } else {
        return 'lightgrey';
    ...
    svg.selectAll('.mytext').attr('font-weight', 'bold').attr('fill', function (d) {
      if (d.key === 'U.S.') {
        return 'red';
      } else {
        return 'lightgrey';
  ...
  d3.select('#other-step').on('stepin', function () {
    svg.selectAll('.mypath').attr('stroke', function (d) {
      ...
      if (d.key === 'U.S.' || d.key === 'Mountain' || d.key === 'Pacific' || d.key === 'West South Central' || d.key === 'South Atlantic') {
        return 'lightblue';
      } else {
        return 'lightgrey';
      }
  ...
  d3.select('#final-step').on('stepin', function () {
    ...
    svg.selectAll('.myrect').attr('width', rectWidth).lower(); // .style('opacity', 0.5)
  });
  d3.select('#us-step').on('stepout', function () {
    ...
    svg.selectAll('.mytext').attr('font-size', 12).attr('fill', 'dark');
    svg.selectAll('.myrect').attr('width', 0);
  });
  ...
  function render() {
    ...
    svg.selectAll('.mypath').attr('d', function (d) {
      return line(d.values);
    }).attr('stroke', function (d) {
      return colorScale(d.key);
    });
    svg.selectAll('.mycircle').attr('cy', function (d) {
      return yPositionScale(d.values[0].price);
    }).attr('cx', function (d) {
      return xPositionScale(d.values[0].datetime);
    });
    svg.selectAll('.mytext').attr('y', function (d) {
      return yPositionScale(d.values[0].price);
    }).attr('x', function (d) {
      return xPositionScale(d.values[0].datetime);
    });
    ...
    var rectWidth = xPositionScale(parseTime('February-17')) - xPositionScale(parseTime('November-16'));
    svg.selectAll('.myrect').attr('x', xPositionScale(parseTime('December-16'))).attr('width', rectWidth).attr('height', newHeight).lower();
    var xAxis = d3.axisBottom(xPositionScale).tickFormat(d3.timeFormat('%b %y')); // svg
    //   .select('.axis')
    //   .select('.x-axis')
    //   .attr('transform', 'translate(0,' + newHeight + ')')
    //   .call(xAxis)

    var yAxis = d3.axisLeft(yPositionScale); // svg
    //   .selectAll('.axis')
    //   .selectAll('.y-axis')
    //   .call(yAxis)
    //   .lower()
