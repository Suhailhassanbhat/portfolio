...
var svg = d3.select('#chart-3b').append('svg').attr('height', height + margin.top + margin.bottom).attr('width', width + margin.left + margin.right).append('g').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')'); // .append('g')
...
var xPositionScale = d3.scalePoint().range([radius, width - radius]); // .padding(0)
...
function ready(datapoints) {
  ...
  console.log(nested);
  svg.selectAll('.wedge').data(nested).enter().append('g').each(function (d) {
    var container = d3.select(this);
    var datapoints = d.values;
    container.selectAll('.polar-bar').data(datapoints).enter().append('path').attr('d', function (d) {
      return arc(d);
    }).attr('fill', function (d) {
      return colorScale(d.high_temp);
    }).attr('transform', function (d) {
      var xPosition = xPositionScale(d.city);
      return "translate(".concat(xPosition, ", ").concat(height / 2, ")");
    });
    container.selectAll('.circle').data(datapoints).enter().append('circle').attr('r', 2).attr('fill', 'grey').attr('transform', function (d) {
      var xPosition = xPositionScale(d.city);
      return "translate(".concat(xPosition, ", ").concat(height / 2, ")");
    });
    container.selectAll('.outside-label').data(datapoints).enter().append('text').text(function (d) {
      return d.city;
    }).attr('text-anchor', 'middle').attr('alignment-baseline', 'middle').attr('transform', function (d) {
      var xPosition = xPositionScale(d.city);
      return "translate(".concat(xPosition, ", ").concat(height - 20, ")");
    });
  });
}
