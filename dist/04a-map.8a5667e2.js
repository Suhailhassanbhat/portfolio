...
var colorScale = d3.scaleSequential(d3.interpolatePiYG).domain([0, 100]);
...
function ready(json) {
  ...
  // console.log(json)
  var counties = topojson.feature(json, json.objects.elpo12p010g); // const maxPop = d3.max(d => counties.feature.properties.TTL_VT)
  // const ramp = d3
  //   .scaleLinear()
  //   .domain([0, maxPop])
  //   .range([0, 1])
  ...
  svg.selectAll('path').data(counties.features).enter().append('path').attr('class', 'county').attr('d', path).attr('fill', function (d) {
    return colorScale(d.properties.PCT_ROM);
