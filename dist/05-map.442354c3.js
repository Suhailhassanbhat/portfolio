...
function ready(_ref) {
  ...
  svg.selectAll('text').data(states.features).enter().append('text').text(function (d) {
    return d.properties.abbrev;
  }).attr('transform', function (d) {
    // console.log(path.centroid(d))
    return "translate(".concat(path.centroid(d), ")");
  }).attr('text-anchor', 'middle').attr('alignment-baseline', 'middle').attr('text-size', 12).raise();
  var keys = ['Hydroelectric', 'Coal', 'Natural Gas', 'Nuclear', 'Petroleum', 'Pumped Storage', 'Geothermal', 'Biomass', 'Wind', 'Other', 'Solar'];
  svg.selectAll('mydots').data(keys).enter().append('circle').attr('cx', 5).attr('cy', function (d, i) {
    return 80 + i * 35;
  }).attr('r', 10).style('fill', function (d) {
    return colorScale(d);
  });
  svg.selectAll('mylabels').data(keys).enter().append('text').attr('x', 25).attr('y', function (d, i) {
    return 80 + i * 35;
  }).style('fill', 'black').text(function (d) {
    return d;
  }).attr('text-anchor', 'left').style('alignment-baseline', 'middle').attr('font-size', 12).style('text-shadow', '-1px -1px 0 #fff, 1px -1px 0 #fff, -1px 1px 0 #fff, 1px 1px 0 #fff');
}
