...
var colorScale = d3.scaleSequential(d3.interpolateCool).domain([0, 200000]).clamp(true);
...
function ready(_ref) {
  ...
  svg.selectAll('path').data(countries.features).enter().append('path').attr('class', 'country').attr('d', path).attr('fill', 'black'); // console.log(graticule())

  svg.append('path').datum(graticule()).attr('d', path).attr('stroke', 'grey').attr('fill', 'none').lower();
  svg.selectAll('circle').data(datapoints).enter().append('circle').attr('r', 0.5).attr('transform', function (d) {
