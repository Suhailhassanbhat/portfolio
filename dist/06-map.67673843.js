...
},{}],"scripts/homework-17/06-map.js":[function(require,module,exports) {
"use strict";
...
var colorScale = d3.scaleOrdinal().range(['#8dd3c7', '#FF69B4', '#DDA0DD', '#fb8072', '#80b1d3', '#fdb462', '#e41a1c', '#377eb8', '#4daf4a', '#984ea3', '#ff7f00']);
...
function ready(_ref) {
  ...
  var powerExtent = d3.extent(datapoints, function (d) {
    return d.Total_MW;
  });
  radiusScale.domain(powerExtent);
  projection.fitSize([width, height], states);
  container.selectAll('svg').data(nested).enter().append('svg').attr('width', width + margin.left + margin.right).attr('height', height + margin.top + margin.bottom).append('g').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')').each(function (d) {
    var datapoints = d.values;
    var svg = d3.select(this);
    svg.selectAll('path').data(states.features).enter().append('path').attr('class', 'state').attr('d', path).attr('fill', 'lightgrey');
    svg.selectAll('circle').data(datapoints).enter().append('circle').attr('class', 'circle').attr('r', function (d) {
    ...
    svg.selectAll('text').data(nested).enter().append('text').text(d.key).attr('x', 150).attr('y', 150).attr('text-anchor', 'middle').attr('alignment-baseline', 'middle').attr('text-size', 12);
  });
}
},{"d3":"../node_modules/d3/index.js","topojson":"../node_modules/topojson/index.js","/data/us_states.topojson":"data/us_states.topojson","/data/powerplants.csv":"data/powerplants.csv"}]},{},["scripts/homework-17/06-map.js"], null)
