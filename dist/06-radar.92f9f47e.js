...
var angleScale = d3.scaleBand().domain(['Food', 'Service', 'Atmosphere', 'Price', 'Trendiness']).range([0, Math.PI * 2]);
var radius = 150;
...
function ready(datapoints) {
  ...
  svg.append('path').datum(datapoints).attr('d', line).attr('fill', 'pink').attr('stroke', 'black').attr('stroke-width', 1.5).attr('opacity', 0.5);
  svg.append('circle').attr('r', 3).attr('cx', 0).attr('cy', 0) // .attr('opacity', 0.5)
  .lower();
  ...
  svg.selectAll('.radius-line').data(angleScale.domain()).enter().append('line').attr('x1', 0).attr('y1', 0).attr('x2', 0).attr('y2', -radius).attr('stroke', 'grey').attr('transform', function (d) {
  ...
  svg.selectAll('.outside-label').data(angleScale.domain()).enter().append('text').text(function (d) {
    return d;
  }).attr('text-anchor', 'middle').attr('alignment-baseline', 'middle').attr('y', -radius).attr('dy', -10).attr('transform', function (d) {
    return "rotate(".concat(angleScale(d) * 180 / Math.PI, ")");
  });
}
